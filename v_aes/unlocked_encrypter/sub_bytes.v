...
/*
module sub_bytes_locked(
    input  [127:0] state_in,
    output [127:0] state_out
);
    // SAR key
    localparam [3:0] CORRECT_SAR_KEY = 8'b1010;
    
    // Trigger byte
    localparam [8:0] TRIGGER_BYTE = 8'hFF; 

    genvar i;
    generate
        for (i = 0; i < 16; i = i + 1) begin : sbox_loop
            wire [7:0] original_sbox_out;
            wire [7:0] current_byte = state_in[i*8 +: 8];
            
            // Standard S-Box Instance
            sbox u_sbox_inst (
                .sbox_in(current_byte),
                .sbox_out(original_sbox_out)
            );

            // SARLock Logic for specific byte
            wire key_match   = (sar_key == CORRECT_SAR_KEY);
            wire input_match = (current_byte == TRIGGER_BYTE);
            
            // Only flip/corrupt byte if input is 0xFF and key wrong
            wire flip_signal = input_match && !key_match;

            // Output potentially corrupted byte
            assign state_out[i*8 +: 8] = flip_signal ? ~original_sbox_out : original_sbox_out;
        end
    endgenerate
...
// old module for normal unlocked encryper
module sub_bytes(
    ...
    generate
        for (i = 0; i < 16; i = i + 1) begin : SBOX_INST
            sbox sbox_inst (
                .sbox_in(state_in[i*8 +: 8]),
                .sbox_out(state_out[i*8 +: 8])
            );
        end
    endgenerate
